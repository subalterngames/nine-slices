...
impl NineSlicedSprite {
    ...
    pub fn resize(&self, size: Size) -> Result<Sprite, Error> {
        ...
        let rect = ClippedRect::new(PositionI::default(), size, outer_size).ok_or(Error::ClippedRect(size))?;

        // Top-left corner.
        let mut top_left = rect.clone();
        top_left.set_src_rect(PositionU::default(), Size {
        ...
        top_right.set_src_rect(PositionU {
            x: x1,
            y: 0,
        }, top_right_size);
        ...
        let bottom_right_size = Size {
            w: w1,
            h: h1
        };
        let y1 = size.h - top_right_size.h;
        bottom_right.set_src_rect(PositionU {
            x: x1,
            y: y1,
        }, bottom_right_size);
    ...
    fn get_dst(&self, size: &Size) -> Vec<u8> {
        ...
        match &self.pixel_type {
            PixelType::Gr8 | PixelType::GrA8 | PixelType::Rgb8 | PixelType::Rgba8 => vec![0; size.w * size.h * self.pixel_type.stride()],
